...
{
    class dkGraniastosłupSześciokątny : dkWielościany
    {//deklaracja zmiennych globalnych dla danej klasy
        ...
        public dkGraniastosłupSześciokątny(int dkR, int dkWysokośćOstrosłupa, int dkStopieńWielokątaPodstawy, int dkXsS, int dkYsS,
            int dkXsP, int dkYsP,
            Color dkKolorLinii, DashStyle dkStylLinii, int dkGrubośćLinii) : base(dkR, dkStopieńWielokątaPodstawy, dkKolorLinii,
                dkStylLinii, dkGrubośćLinii)
        {//odziedziczenie wartości zmiennych z klasy nadrzędnej
            this.dkrodzaj_bryły = dktyp_bryły.dkbg_graniastosłup_sześciokątny;
            ...
            float dkBokA = (float)(dkOś_duża / Math.Sqrt(2));
            float dkPolePodstawy = (dkBokA * 2);
            float dkPoleBoku = (dkBokA * dkWysokośćBryły);
            this.dkObjętośćBryły = (dkPolePodstawy * dkWysokośćBryły);
            this.dkPowierzchniaBryły = (dkPolePodstawy * 2 + dkPoleBoku * 4);
        }
        public override void dkWykreśl()
        {
            ...
            for (int dki = 0; dki < dkWielokątPodłogi.Length - 1; dki++)
            {
                ...
                Point dkWspółrzędneSufitu = new Point(dkWielokątPodłogi[dki].X, dkWielokątPodłogi[dki].Y - dkWysokośćBryły);
                Point dkWspółrzędneSufitu2 = new Point(dkWielokątPodłogi[dki + 1].X, dkWielokątPodłogi[dki + 1].Y - dkWysokośćBryły);
                dkRysownica.DrawLine(dkPióro, dkWielokątPodłogi[dki], dkWspółrzędneSufitu);
                dkRysownica.DrawLine(dkPióro, dkWspółrzędneSufitu, dkWspółrzędneSufitu2);
            }
            //zwolninie pióra
            dkPióro.Dispose();
        }
        ...
        {/*Wymazuje poprzednią figurę, ustawia wszsytkieparametry na nowe, podane z klasy głownek
            ...
            dkWymaż();
            this.dkXsP = dkx - (int)(this.dkWysokośćBryły / (Math.Tan(Math.PI * dkKątPochylylenia / 180f)));
            this.dkYsP = dky;
