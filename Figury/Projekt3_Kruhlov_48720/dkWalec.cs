...
{
    class dkWalec : dkbryła_abstrakcyjna
    {//deklaracja zmiennych globalnych dla danej klasy
        public int dkStopieńWielokątaPodstawy;
        float dkOś_duża, dkOś_mała;
        float dkKątMiędzyWierzchołkami, dkKątPołożenia;

        Point[] dkWielokątPodłogi;
        Point[] dkWielokątSufitu;
        public dkWalec(int dkR, int dkWysokość, int dkStopieńWielokątaPodstawy, int dkXsS, int dkYsS, int dkXsP, int dkYsP, Color dkKolorLinii,
            DashStyle dkStylLinii, int dkGrubośćLinii) : base(dkR, dkKolorLinii, dkStylLinii, dkGrubośćLinii)
        {//odziedziczenie wartości zmiennych z klasy nadrzędnej
            this.dkrodzaj_bryły = dktyp_bryły.dkbg_walec;
            this.dkWysokośćBryły = dkWysokość;
            ...
            this.dkWielokątSufitu = new Point[dkStopieńWielokątaPodstawy];
            ...
            for (int dki = 0; dki < dkStopieńWielokątaPodstawy; dki++)
            {
                ...
                dkWielokątSufitu[dki] = new Point();
                ...
                dkWielokątSufitu[dki].X = dkWielokątPodłogi[dki].X;
                dkWielokątSufitu[dki].Y = dkWielokątPodłogi[dki].Y - dkWysokośćBryły;
            }
            //obliczenie objętości i pola powierzchni figury
            float dkPolePodstawy = (float)(Math.PI * Math.Pow(dkOś_duża / 2, 2));
            float dkPoleBoku = (float)(2 * Math.PI * dkOś_duża / 2 * dkWysokośćBryły);
            this.dkObjętośćBryły = dkPolePodstawy * dkWysokośćBryły;
            this.dkPowierzchniaBryły = (2 * dkPolePodstawy + dkPoleBoku);
        }
        public override void dkWykreśl()
        {
            ...
            dkRysownica.DrawEllipse(dkPióro, this.dkXsS - this.dkOś_duża / 2,
                this.dkYsS - this.dkOś_mała / 2,
                this.dkOś_duża, this.dkOś_mała);
            dkRysownica.DrawLine(dkPióro, dkXsP - dkOś_duża / 2, dkYsP, dkXsS - dkOś_duża / 2, dkYsS); //dobrze
            dkRysownica.DrawLine(dkPióro, dkXsP + dkOś_duża / 2, dkYsP, dkXsS + dkOś_duża / 2, dkYsS); //dobrze
            //wykreślenie na ścianiebocznej tego czegoś
            for (int dki = 0; dki < dkStopieńWielokątaPodstawy; dki++)
            {
                dkRysownica.DrawLine(dkPióro, dkWielokątPodłogi[dki], dkWielokątSufitu[dki]);
            }
            dkPióro.Dispose();
        }
        ...
        public override void dkObróć_i_wykreśl(float dkrotateAngle)
        {
            ...
            for (int dki = 0; dki < dkStopieńWielokątaPodstawy; dki++)
            {
                ...
                dkWielokątPodłogi[dki].X = (int)(this.dkXsP + dkOś_duża / 2 * Math.Cos(Math.PI * (dkKątPołożenia + dki * dkKątMiędzyWierzchołkami) / 180f));
                dkWielokątPodłogi[dki].Y = (int)(this.dkYsP + dkOś_mała / 2 * Math.Sin(Math.PI * (dkKątPołożenia + dki * dkKątMiędzyWierzchołkami) / 180f));
        ...
        public override void dkUstawStopieńWielokąta(int dkStopień)
        {
            ...
            this.dkWielokątPodłogi = new Point[dkStopień];
            this.dkWielokątSufitu = new Point[dkStopień];
        ...
        public override void dkUstawKątNachyleniaStożka(int X) { /*pusta implementacja */ }
