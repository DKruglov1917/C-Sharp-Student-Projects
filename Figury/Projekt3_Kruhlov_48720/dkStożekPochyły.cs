...
{
    class dkStożekPochyły : dkWielościany
    {//deklaracja zmiennych globalnych dla danej klasy
        ...
        Point dkSufit;
        int dkZmiennaPomocnicza;
        public dkStożekPochyły(int dkR, int dkWysokośćStożka, int dkStopieńWielokątaPodstawy, int dkXsS, int dkYsS, int dkXsP, int dkYsP,
            Color dkKolorLinii, DashStyle dkStylLinii, int dkGrubośćLinii) : base(dkR, dkStopieńWielokątaPodstawy, dkKolorLinii, dkStylLinii, dkGrubośćLinii)
        {//odziedziczenie wartości zmiennych z klasy nadrzędnej
            this.dkrodzaj_bryły = dktyp_bryły.dkbg_stożek_pochyły;
            dkKątNachylenia = Math.Abs(dkXsP - dkXsS);
            dkZmiennaPomocnicza = dkXsP;
            ...
            for (int dki = 0; dki < dkStopieńWielokątaPodstawy; dki++)
            {
                ...
                dkWielokątPodłogi[dki].X = (int)(this.dkXsP + dkOś_duża / 2 * Math.Sin(Math.PI * (dkKątPołożenia + dki * dkKątMiędzyWierzchołkami) / 180f));
                dkWielokątPodłogi[dki].Y = (int)(this.dkYsP + dkOś_mała / 2 * Math.Cos(Math.PI * (dkKątPołożenia + dki * dkKątMiędzyWierzchołkami) / 180f));
            }
        ...
        {/*Wymazuje poprzednią figurę, ustawia wszsytkieparametry na nowe, podane z klasy głownek
            ...
            for (int dki = 0; dki < dkStopieńWielokątaPodstawy; dki++)
            {
                dkWielokątPodłogi[dki].X = (int)(this.dkXsP + dkOś_duża / 2 *
                    Math.Cos(Math.PI * (dkKątPochylylenia + dki * dkKątMiędzyWierzchołkami) / 180f));
        ...
        public override void dkUstawKątNachyleniaStożka(int dkX)
        {
            dkXsP = -dkX + dkZmiennaPomocnicza + 90;
            dkKątNachylenia = Math.Abs(dkXsP - dkXsS);
        }
