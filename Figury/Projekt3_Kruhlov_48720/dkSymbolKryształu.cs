...
{
    class dkSymbolKryształu : dkWielościany
    {//deklaracja zmiennych globalnych dla danej klasy
        ...
        public dkSymbolKryształu(int dkR, int dkWysokośćOstrosłupa, int dkStopieńWielokątaPodstawy, int dkXsS, int dkYsS, int dkXsP, int dkYsP,
        ...
        {//odziedziczenie wartości zmiennych z klasy nadrzędnej
            this.dkrodzaj_bryły = dktyp_bryły.dkbg_symbol_kryształu;
            ...
            this.dkObjętośćBryły = (dkPolePodstawy * dkWysokośćBryły * 2 / 3);
            this.dkPowierzchniaBryły = (dkPoleBoku * 12);
        }
        public override void dkWykreśl()
        {
            ...
            for (int dki = 0; dki <= dkStopieńWielokątaPodstawy; dki++)
            {
                dkRysownica.DrawLine(dkPióro, dkWielokątPodłogi[dki], new Point(dkXsS, dkYsP - dkWysokośćBryły));
                dkRysownica.DrawLine(dkPióro, dkWielokątPodłogi[dki], new Point(dkXsP, dkYsP + dkWysokośćBryły));
            }
            //zwolninie pióra
            dkPióro.Dispose();
        }
